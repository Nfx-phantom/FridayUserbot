...
CUSTOM_STICKER_NAME =str(CUSTOM_STICKER_PACK_NAME) if CUSTOM_STICKER_PACK_NAME else "My Boss Phantom Volume Pack One"
CUSTOM_ANIME_PACK = str(CUSTOM_ANIMATED_PACK_NAME) if CUSTOM_ANIMATED_PACK_NAME else "My Boss Animated Pack"
FILLED_UP_DADDY = "Invalid pack selected."

@borg.on(admin_cmd(pattern="choori ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    if not event.is_reply:
        await event.edit("Dimag ghutno me hai kya be tera ? Bina sticker ko reply kiye command kaise use hogi ...Chal ab sticker ko reply kar.")
        return
    ...
    #packname = f"PHANTOM_PACK"
    #packshortname = f"PHANTOM_{userid}_ns"  # format: Uni_Borg_userid
    if userid == 1263617196:
        packname = f"@NfxGang Packs 😈"
        packshortname = "NfxPhantomGangPack"
    else:
        packname = f"{user.username}'s {pack}"
        packshortname = f"Phantom_{userid}_Pack"
    await event.edit("`Merko sticker pasand aaya ...abhi add karne de isko ! Tham jaa thodi der bas hogya kang ?`")
    ...
    if is_a_s:
        ...
        if userid == 813878981:
            packname = f"NfxPhantomGang Ka Pack"
            packshortname = "Nfx Gang Is Great"
        else:
            packname = f"{user.username}'s {pack}"
    ...
    await event.edit("Add horha hai wait karo ! Thamba Thamba!")

    async with borg.conversation("@Stickers") as bot_conv:
        now = datetime.datetime.now()
        dt = now + datetime.timedelta(minutes=1)
        if not await stickerset_exists(bot_conv, packshortname):
            await event.edit("`Pack nahi bnaya ghochu tune ...Naya pack bnane de !`")
            ...
            if response.text == FILLED_UP_DADDY:
                while response.text == FILLED_UP_DADDY:
                    ...
                    #if userid == 948408212:
                       # packname = f"{user.username}'s {pack}"
                       # packshortname = "Vol._{pack}_PHANTOM_ke_locker_me"
                   # else:
                       # packname = f"Vol._{pack}_PHANTOM{userid}"
                        #packshortname = f"Vol._{pack}_PHANTOM_{userid}_ns"
                    if not await stickerset_exists(bot_conv, packshortname):
                ...
                await silently_send_message(bot_conv, sticker_emoji)
                await silently_send_message(bot_conv, "/done")
    await event.edit(f"**Sticker add hogya chal ab hss de .Tera sticker will be found [Here](t.me/addstickers/{packshortname})")
